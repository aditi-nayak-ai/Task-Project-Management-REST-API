...
def update_task(task_id: int, payload: TaskUpdate, db: Session = Depends(get_db), current_user: User = Depends(require_role("admin", "manager"))):
    # FOR UPDATE row lock: without it, two PATCHes carrying the same version can
    # both pass the check below and both commit (a lost update). With it, the
    # second request waits, then re-reads the bumped version and gets a 409.
